...
export class AuthService {
  ...
  getTokenSilently$(options?): Observable<string> {
    ...
    return this.auth0Client$.pipe(
      ...
      concatMap((client: Auth0Client) => from(client.getTokenSilently(newOptions)))
    );
  }

  getTokenSilentlyAsPromise$(options?): Promise<string> {
    return new Promise((resolve, reject) =>
      this.getTokenSilently$(options).subscribe(resolve, reject)
    );
  }
